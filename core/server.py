...
class Server(object):
    ...
    def train(self):
        ...
        if self.args.unlearningChoice == False: 
            ...
            if self.args.unlearningMethods == "cont_w":
                ...
                for e in range(self.args.unlearningEpochs):
                    ...
                    with alive_bar(len(self.clients)) as bar:
                        ...
                        for client in self.clients:
                            if client.clientIndex == self.unlearningClientIndex:
                                _, _ = client.trainCan(getParameter(self.globalModel), self.initModel, self.pretrainModel)
                            else:
                                _, _ = client.train(getParameter(self.globalModel))
                            bar()
                ...
                for e in range(self.args.unlearningEpochs):
                    ...
                    with alive_bar(len(self.clients)) as bar:
                        ...
                        for client in self.clients:
                            if client.clientIndex == self.unlearningClientIndex:
                                _, _ = client.trainAsc(getParameter(self.globalModel))
                            else:
                                _, _ = client.train(getParameter(self.globalModel))
                            bar()
                ...
                for e in range(self.args.unlearningEpochs):
                    ...
                    with alive_bar(len(self.clients)) as bar:     
                        for client in self.clients:
                            if client.clientIndex == self.unlearningClientIndex:
                                loss_, accuracy_ = client.trainActive(getParameter(self.globalModel))
                            else:
                                loss_, accuracy_ = client.train(getParameter(self.globalModel))
                                globalTrainLoss += loss_
                                globalTrainAccuracy += accuracy_
                ...
                for e in range(self.args.unlearningEpochs):
                    ...
                    with alive_bar(len(self.clients[1:])) as bar:     
                        for client in self.clients[1:]:
                            loss_, accuracy_ = client.trainRip(getParameter(self.globalModel))
                            globalTrainLoss += loss_
                            globalTrainAccuracy += accuracy_
                            bar()
            ...
            elif self.args.unlearningMethods == "ffmu":
                print("Pre-Train Test:")
                self.test(-1)
                for e in range(self.args.unlearningEpochs):
                    print("*************start unlearning in Eopchs:{}*************".format(e))
                    with alive_bar(len(self.clients)) as bar:
                        for client in self.clients:
                            loss_, accuracy_ = client.trainFFMU(getParameter(self.globalModel))
                            bar()
            ...
            elif self.args.unlearningMethods == "ifu":
                # Init model
                self.globalModel = copy.deepcopy(self.initModel)

                sigma = 0.1
                epsilon = 10
                psiThr = (math.pow(2*(np.log(1.25)-np.log(0.1)), -0.5))*sigma*epsilon
                # psiSum = np.sqrt(2 * np.log(1.25 / 0.1))
                psiSum = 0
                print("psiThr:", psiThr, "   psiSum:", psiSum)
                globalModels = []

                for g in range(50):
                    print("*************Pre-training in Eopchs:{}*************".format(g))
                    ...
                    with alive_bar(self.numClient) as bar:
                        for client in self.clients:
                            loss_, accuracy_, psi = client.trainIFU(getParameter(self.globalModel), g)
                    ...
                    self.test(g)
                    if psiSum<psiThr:
                        globalModels.append(self.globalModel.state_dict())
                    else: 
                        break
                   
                print("*************Start Unlearning with IFU*************")
                self.globalModel.load_state_dict(globalModels[-1])

                # print noised accuracy
                print("---------------------Add Noise----------------------")
                # sigma = math.sqrt(2*(math.log(1.25)-math.log(0.1)))*psiSum/epsilon
                noiseModel(self.globalModel, sigma)
                self.globalModel.to(self.device)
                self.test(-1)
                print("----------------------------------------------------")

                retrainRound = 40
                for e in range(retrainRound):
                    print("************ ReTrain in Eopchs:{}*************".format(e))
                    globalTrainLoss = 0.0
                    globalTrainAccuracy = 0.0
                    ...
                    self.globalModel.load_state_dict(aggregateFedAvg(self.clients[1:], self.dataloader.totalTrain, True))
